...
void server_start(int port) {
    ...
    if (bind(sockfd, (const struct sockaddr *)&server_addr, sizeof(server_addr)) < 0) {
        fprintf(stderr, "Bind failed. Note: Binding to Port 53 usually requires Admin/Root privileges.\n");
        closesocket(sockfd);
        return;
    }
    ...
    while (1) {
        ...
        if (query_end > 0) {
            char ipv4[16];
            const char *ip_to_send = NULL;
            
            // 1. Try Memory Cache
            if (cache_get(domain, ipv4, sizeof(ipv4))) {
                ip_to_send = ipv4;
                printf("Query: %s -> %s (Cache Hit)\n", domain, ipv4);
            } else {
                // 2. Fallback to SQLite DB
                if (db_query(domain, ipv4, sizeof(ipv4))) {
            ...
            size_t resp_len = protocol_build_response(buffer, query_end, ip_to_send);
            sendto(sockfd, (const char *)buffer, (int)resp_len, 0, (const struct sockaddr *)&client_addr, len);
        }
    }
